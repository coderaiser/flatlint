...
test('flatlint: with-plugins: convert-comma-to-semicolon: after arrow', (t) => {
    const source = 'const condition = (path) => isObjectInsideArrow(path),\n';
    const [code] = lint(source, {
        plugins,
    });
    
    const expected = 'const condition = (path) => isObjectInsideArrow(path);\n';
...
test('flatlint: with-plugins: split-namespace-to-specifiers', (t) => {
    const source = montag`
...
test('flatlint: with-plugins: remove-useless-import-curly-braces', (t) => {
    const source = montag`
...
test('flatlint: with-plugins: apply-decorator-export', (t) => {
    const source = montag`
...
test('flatlint: with-plugins: apply-decorator-private', (t) => {
    const source = montag`
...
test('flatlint: with-plugins: add-missing-const', (t) => {
    const source = montag`
        export {scan, fix, report} = matchFiles();
    `;
    ...
    const expected = montag`
        export const {scan, fix, report} = matchFiles();
    `;
