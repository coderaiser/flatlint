...
test('flatlint: with-plugins: remove-useless-semicolon', (t) => {
    const source = `export; const isTemplateTail = (a) => a?.type === 'TemplateTail';`;
    const [result] = lint(source, {
        plugins,
    });
    
    const expected = `export const isTemplateTail = (a) => a?.type === 'TemplateTail';`;
