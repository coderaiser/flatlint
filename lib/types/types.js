...
export const createIsType = (type) => (token, tokens) => {
    if (!token)
        return false;
    ...
    for (const {value} of maybeArray(tokens)) {
...
export const isPunctuator = createIsType('Punctuator');
export const isInvalid = createIsType('Invalid');
...
const createIsBracesBalanced = ({open, close}) => (path) => {
    let balance = 0;
...
export const isSquireBracesBalanced = createIsBracesBalanced({
    open: openSquireBrace,
    close: closeSquareBrace,
});

export const isRoundBracesBalanced = createIsBracesBalanced({
    open: openRoundBrace,
